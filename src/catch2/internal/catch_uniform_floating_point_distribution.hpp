 ...
 *
 * Note that we support only `float` and `double` types, because the standard
 * does not provide us with wide-enough integer types to handle bigger
 * floating point types. Also note that we don't implement all parts of
